...
import { buildObjectRefinementPrompt } from "./refinePrompt.js";
...
app.post("/api/refine-object", async (request, response) => {
  const { apiKey, model = "gpt-5.5", element, instructionNodes = [], edges = [] } = request.body;
  if (!apiKey) return response.status(400).json({ error: "Add an OpenAI API key in OpenAI settings first." });
  if (!element || element.type !== "text") return response.status(400).json({ error: "Select a text box first." });

  const { count, prompt } = buildObjectRefinementPrompt({ element, instructionNodes, edges });
  try {
